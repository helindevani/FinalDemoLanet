...
const rout = useRouter();
  ...
  const submitHandler = async (event: React.FormEvent<HTMLFormElement> | any) => {
          ...
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <form className="space-y-4 md:space-y-6" onSubmit={submitHandler}>
              ...
              <div className="flex items-center justify-between">
                ...
                <Link href="forgotpassword" className="text-sm font-medium text-orange-600 hover:underline dark:text-primary-500">Forgot password?</Link>
              </div>
              <button type="submit" className="w-full text-white bg-orange-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Sign in</button>
              <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                Don’t have an account yet? <Link href="register" className="font-medium text-orange-600 hover:underline dark:text-primary-500">Sign up</Link>
              </p>
            </form>
          </div>
        </div>
